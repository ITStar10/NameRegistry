...
describe("NameRegistry", function () {
  ...
  it ("Register", async function() {
    await contract.register(testNames[0], testDIDs[0]);

    // Invalid address
    await expect(contract.register(testNames[0], '0x0')).to.be.rejectedWith('invalid address');
    // Zero address
    await expect(contract.register(testNames[0], zeroAddress)).to.be.rejectedWith('Invalid zero address');
    // Duplcation Name
    await expect(contract.register(testNames[0], testDIDs[1])).to.be.rejectedWith('Name already registered');
    // Duplication DID
    await expect(contract.register(testNames[1], testDIDs[0])).to.be.rejectedWith('DID already registered');
    // Case-sensitive test
    let uppercaseDID = testDIDs[0].toUpperCase().substring(2);
    uppercaseDID = '0x' + uppercaseDID;
    await expect(contract.register(testNames[1], uppercaseDID)).to.be.rejectedWith('DID already registered');
  })
  ...
  it ("Find Test", async function() {
    // Register for Test
    for (let i = 0; i < 2; i++) {
      await contract.register(testNames[i], testDIDs[i]);
    }
